...
export default function TaskOutcomeArchive({ taskId, outcome: initial, canUpload, canManage, currentUserId }: Props) {
  ...
  async function upload(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        setOutcome((old) => old ? { ...old, assets: [...old.assets, data.asset] } : { id: "new", title: "", submittedAt: new Date().toISOString(), summary: null, externalUrl: null, externalLabel: null, updatedAt: new Date().toISOString(), updatedBy: { id: currentUserId, displayName: "我" }, assets: [data.asset] });
      }
  ...
  async function saveMeta() {
    setSaving(true);
    try {
      const res = await fetch(`/api/tasks/${taskId}/outcome`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ title: title || null, summary: summary || null, externalUrl: externalUrl || null, externalLabel: externalLabel || null }) });
  ...
  return <Card title={outcome?.title || "成果档案"} extra={<Space>{canUpload && <><input ref={inputRef} type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime,application/pdf" onChange={(e) => void upload(e.target.files)} style={{ display: "none" }} /><Button icon={<UploadOutlined />} loading={saving} onClick={() => inputRef.current?.click()}>归档材料</Button></>}{canManage && <Button onClick={() => setEditing(true)}>编辑档案</Button>}</Space>}>
    ...
    {outcome && <Typography.Text type="secondary" style={{ display: "block", marginTop: 16, fontSize: 12 }}>提交时间：{new Date(outcome.submittedAt).toLocaleString()} · 最后整理：{outcome.updatedBy.displayName} · {new Date(outcome.updatedAt).toLocaleString()}</Typography.Text>}
    <Modal title="编辑成果档案" open={editing} onCancel={() => setEditing(false)} onOk={() => void saveMeta()} confirmLoading={saving} okText="保存"><Typography.Paragraph type="secondary">命名规范：姓名＋活动名称＋微推/海报/视频/其他。链接可填写公众号推文、视频号、网盘或线上成品页面。</Typography.Paragraph><Input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={191} placeholder="例如：张三+迎新志愿活动+微推" /><Input.TextArea value={summary} onChange={(e) => setSummary(e.target.value)} rows={5} maxLength={5000} placeholder="简要说明本任务产出了什么、使用场景和完成情况" style={{ marginTop: 12 }} /><Input value={externalLabel} onChange={(e) => setExternalLabel(e.target.value)} maxLength={120} placeholder="链接名称，例如：公众号推文" style={{ marginTop: 12 }} /><Input value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} placeholder="https://..." style={{ marginTop: 8 }} /></Modal>
  </Card>;
}
