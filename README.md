...
- **成员个人主页** — 任务详情页点头像即进；活动记录**按月折叠**，展开看当月具体任务与积分、会议旷会扣分
- **个人课程表** — 支持教务系统 **PDF 课表**（纯算法解析，无需 OCR / 外部 API）、JSON 导入（或粘贴 JSON）、手动增删改、按周查看，按学期起始日自动定位当前周；考勤页首个 Tab 即为课程表
...
课程表与小程序启动页功能的实现记录见 [课程表功能开发报告](docs/COURSE_SCHEDULE_REPORT.md)。
成员个人主页（按月折叠活动明细）的实现记录见 [个人主页开发报告](docs/PROFILE_MONTHLY_REPORT.md)。
...
后续开发或交接给 AI 前，请先阅读 [AI 开发交接说明](AI_HANDOFF.md)。其中以当前代码和生产拓扑为准，包含数据库迁移、Web/小程序对齐、SSH 运维与上线验收流程。

> 注：`docs/AI_HANDOFF.md` 是过时副本，请以根目录 `AI_HANDOFF.md` 为准。
